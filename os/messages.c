...
//есть неомер события и ссылка на функцию. 
...
// установить таймер
// пример вызова: setTimer(MSG_LCD_REFRESH, 0, 50);  (ВЫЗЫВАЕМОЕ,? , время в мс)
...
void initMessages() {
	...
	// главный таймер
	TCCR2 = _BV(WGM21) | _BV(CS22);  //256  сброс при совпадении
	OCR2 = 124; // период 1/100 сек, делитель 1/1024
	TIMSK |= _BV(OCIE2);

	
	/*
	TIMSK |= (1<<TOIE1) | (1<<OCIE1B)| (1<<OCI 6E1A) ;  //включение прерываний таймера
	TIFR &=~ (1<<TOV1); //вектор прерывание по 1- совпадение
